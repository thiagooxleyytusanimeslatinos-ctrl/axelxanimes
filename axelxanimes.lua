...
-- ============ ANTI-OBFUSCATION STRINGS ============
local function S(c) return string.char(unpack(c)) end
local STRINGS = {
	-- Script names
	Animate = S({65, 110, 105, 109, 97, 116, 101}),
	Anim = S({65, 110, 105, 109}),
	Handler = S({72, 97, 110, 100, 108, 101, 114}),
	Movement = S({77, 111, 118, 101, 109, 101, 110, 116}),
	Sprint = S({83, 112, 114, 105, 110, 116}),
	-- Part/Object names
	Torso = S({84, 111, 114, 115, 111}),
	Root = S({82, 111, 111, 116}),
	MainPart = S({77, 97, 105, 110, 80, 97, 114, 116}),
	Body = S({66, 111, 100, 121}),
	Chassis = S({67, 104, 97, 115, 115, 105, 115}),
	RootPart = S({82, 111, 111, 116, 80, 97, 114, 116}),
	PrimaryPart = S({80, 114, 105, 109, 97, 114, 121, 80, 97, 114, 116}),
	Handle = S({72, 97, 110, 100, 108, 101}),
	VehicleSeat = S({86, 101, 104, 105, 99, 108, 101, 83, 101, 97, 116}),
	Seat = S({83, 101, 97, 116}),
	RightHand = S({82, 105, 103, 104, 116, 72, 97, 110, 100}),
	RightArm = S({82, 105, 103, 104, 116, 32, 65, 114, 109}),
	NPC = S({78, 80, 67}),
	-- Animation names (inside Animate script)
	idle = S({105, 100, 108, 101}),
	walk = S({119, 97, 108, 107}),
	run = S({114, 117, 110}),
	jump = S({106, 117, 109, 112}),
	fall = S({102, 97, 108, 108}),
	climb = S({99, 108, 105, 109, 98}),
	swim = S({115, 119, 105, 109}),
	swimidle = S({115, 119, 105, 109, 105, 100, 108, 101}),
	sit = S({115, 105, 116}),
	strafe = S({115, 116, 114, 97, 102, 101}),
	swimming = S({115, 119, 105, 109, 109, 105, 110, 103}),
	movement = S({109, 111, 118, 101, 109, 101, 110, 116}),
	dash = S({100, 97, 115, 104}),
	sprint = S({115, 112, 114, 105, 110, 116}),
	WalkAnim = S({87, 97, 108, 107, 65, 110, 105, 109}),
	RunAnim = S({82, 117, 110, 65, 110, 105, 109}),
	JumpAnim = S({74, 117, 109, 112, 65, 110, 105, 109}),
	FallAnim = S({70, 97, 108, 108, 65, 110, 105, 109}),
	ClimbAnim = S({67, 108, 105, 109, 98, 65, 110, 105, 109}),
	SwimAnim = S({83, 119, 105, 109, 65, 110, 105, 109}),
	SwimIdleAnim = S({83, 119, 105, 109, 73, 100, 108, 101, 65, 110, 105, 109}),
	Animation1 = S({65, 110, 105, 109, 97, 116, 105, 111, 110, 49}),
	Animation2 = S({65, 110, 105, 109, 97, 116, 105, 111, 110, 50}),
	-- Keywords for name matching
	cutscene = S({99, 117, 116, 115, 99, 101, 110, 101}),
	scene = S({115, 99, 101, 110, 101}),
	event = S({101, 118, 101, 110, 116}),
	elevator = S({101, 108, 101, 118, 97, 116, 111, 114}),
	intro = S({105, 110, 116, 114, 111}),
	cinematic = S({99, 105, 110, 101, 109, 97, 116, 105, 99}),
	search = S({115, 101, 97, 114, 99, 104}),
	open = S({111, 112, 101, 110}),
	car = S({99, 97, 114}),
	vehic = S({118, 101, 104, 105, 99}),
	chassis_lower = S({99, 104, 97, 115, 115, 105, 115}),
	Crouching = S({67, 114, 111, 117, 99, 104, 105, 110, 103}),
}

local R6_FLIGHT_ANIMS = { -- Claves a prueba de ofuscación
...
-- ANIMACIONES
local ANIMATIONS = { -- Claves a prueba de ofuscación
...
do
	...
	local extraIds = {flightIdleAnim, "93032051646965", flightForwardAnim, flightFastAnim, flightSlowAnim, flightBackwardAnim, "138992096476836", "84043660421785", "90584560149153", "111690963588496", "128406664848479", "140704739422954", "83860986564910", "103145593690285", "128578785610052", "77128372412361", "72301599441680", "87088218490918", "104687069461693", "142495255", "248336294", "54513258", "95424077", "97170520", "97172005", "21633130", "48957148", "56146409", "66703954", "73033633", "97171309", "133479030483520", "117251315086498", "129768396663808",
...
local function canGrabObject(part)
	...
	local function getVehicleRootPart(targetPart)
		...
		local vehicleParts = {STRINGS.MainPart, STRINGS.Body, STRINGS.Chassis, STRINGS.RootPart, STRINGS.PrimaryPart, STRINGS.Handle, STRINGS.VehicleSeat, STRINGS.Seat}
    ...
    if model and (model:FindFirstChildWhichIsA(STRINGS.VehicleSeat, true) or model:FindFirstChildWhichIsA(STRINGS.Seat, true) or
        model.Name:lower():find(STRINGS.car) or model.Name:lower():find(STRINGS.vehic) or model.Name:lower():find(STRINGS.chassis_lower)) then
        isVehicle = true
    end
		...
		for part, props in pairs(data.savedPhysics) do
			if part and part.Parent then
				...
				for _, child in ipairs(part:GetChildren()) do
					if child:IsA("NoCollisionConstraint") and child.Name == "SuperGrabNC" then -- This name is internal to the script, safe.
						child:Destroy()
					end
				end
			end
...
-- ==========================================================
local abilities = { -- Estructura a prueba de ofuscación (índices numéricos)
	{"Espejismo", "miragespeed"},
	{"Super Audición", "superhearing"},
	{"Gravedad", "gravjump"},
	{"Super Salto", "supersalto"},
	{"Rayos X", "xrayvision"},
	{"Velocidad", "walkspeed"},
	{"Vuelo", "flight"},
	{"Super Agarre", "superstrength"},
	{"Re.Instantanea", "instantreaction"}
}

local function isAbilityAvailable(key)
	if isR6 and (key == "miragespeed" or key == "supersalto") then return false end -- Los valores de cadena son seguros
...
local function disableDefaultAnimate()
	local function process(parent)
		...
		for _, scr in ipairs(parent:GetChildren()) do
			if scr:IsA("LocalScript") and (scr.Name:find(STRINGS.Animate) or scr.Name:find(STRINGS.Anim) or scr.Name:find(STRINGS.Handler) or scr.Name:find(STRINGS.Movement) or scr.Name:find(STRINGS.Sprint)) then
				scr.Disabled = true
			end
		end
	end
...
local function startFlight()
	...
	local flightUpdate = RunService.RenderStepped:Connect(function(deltaTime)
		...
		if not isR6 then
			local animate = character:FindFirstChild(STRINGS.Animate)
			if animate then
				local equippedTool = character:FindFirstChildOfClass("Tool")
				local isHoldingTool = equippedTool ~= nil or false
				if not isHoldingTool then
					local rightHand = character:FindFirstChild(STRINGS.RightHand) or character:FindFirstChild(STRINGS.RightArm)
					if rightHand then
...
local function muteAmbientSounds()
	...
	local function processSound(sound)
		...
		local keywords = {STRINGS.music, STRINGS.ambien, STRINGS.radio, STRINGS.soundtrack, STRINGS.song, STRINGS.bgm, STRINGS.theme, STRINGS.boombox, STRINGS.ambiance}
		-- Un sonido se considera música ambiental si es largo, O es un bucle largo, O su nombre sugiere que es música.
		local isLikelyMusic = (sound.TimeLength > 25) or (sound.Looped and sound.TimeLength > 5)
		if not isLikelyMusic then for _, k in ipairs(keywords) do if S_match(soundName, k) then isLikelyMusic = true; break; end end end
...
local function disableFootstepSounds()
	...
	for _, descendant in ipairs(character:GetDescendants()) do
		if descendant:IsA("Sound") then
			...
			if soundName:find(STRINGS.foot) or soundName:find(STRINGS.step) or soundName:find(STRINGS.walk) or soundName:find(STRINGS.run) then
	...
	if humanoid then
		local animateScript = character:FindFirstChild(STRINGS.Animate)
    ...
    pcall(function()
        for i, v in ipairs(game:GetService("Players").LocalPlayer.PlayerGui:GetDescendants()) do
            if v:IsA("LocalScript") then
				...
				local keywords = {STRINGS.anti, STRINGS.cheat, STRINGS.kick, STRINGS.exploit, STRINGS.ban}
				for _, k in ipairs(keywords) do if scriptName:find(k) then v.Disabled = true; break; end end
            end
        end
		...
		for _, desc in ipairs(Workspace:GetDescendants()) do
			if desc:IsA("Humanoid") and desc.Parent and desc.Parent:IsA("Model") then
				...
				if not Players:GetPlayerFromCharacter(model) and model:FindFirstChildWhichIsA("BasePart") then
					...
					if not seekIsActive and model.Name:lower():find(STRINGS.seek) then
						seekIsActive = true
					end
				end
			end
			if not correctDoor and desc:IsA("Model") and desc.Name:lower():find(STRINGS.door) then
...
local function updateXRayVision()
	...
	if now - XRayVision.LastScanTime > 5 then
		...
		local itemWhitelist = {
			S({107, 101, 121}), S({108, 105, 118, 101, 104, 105, 110, 116, 98, 111, 111, 107}), S({108, 101, 118, 101, 114}), S({115, 119, 105, 116, 99, 104}), S({98, 114, 101, 97, 107, 101, 114}), S({98, 114, 101, 97, 107, 101, 114, 98, 97, 110, 107}), S({102, 117, 115, 101}), S({102, 117, 115, 101, 105, 116, 101, 109}), S({108, 105, 118, 101, 102, 117, 115, 101}),
			S({98, 114, 101, 97, 107, 101, 114, 98, 111, 120}), S({101, 108, 101, 99, 116, 114, 105, 99, 97, 108, 98, 111, 120}), S({103, 111, 108, 100}), S({103, 111, 108, 100, 112, 105, 108, 101}), S({108, 105, 103, 104, 116, 101, 114}), S({102, 108, 97, 115, 104, 108, 105, 103, 104, 116}), S({99, 114, 117, 99, 105, 102, 105, 120}), S({108, 111, 99, 107, 112, 105, 99, 107}),
			S({99, 97, 110, 100, 108, 101}), S({98, 97, 116, 116, 101, 114, 121}), S({115, 104, 101, 97, 114, 115}), S({118, 105, 116, 97, 109, 105, 110, 115}), S({98, 97, 110, 100, 97, 103, 101}), S({98, 97, 110, 100, 97, 103, 101, 115}), S({109, 101, 100, 107, 105, 116}), S({112, 114, 101, 115, 101, 110, 116}), S({103, 105, 102, 116})
		}
		...
		for _, v in ipairs(Workspace:GetDescendants()) do
			...
			elseif v:IsA("Model") then
				local monsterKeywords = {STRINGS.rush, STRINGS.ambush, STRINGS.seek, STRINGS.figure, STRINGS.halt, STRINGS.eyes, STRINGS.screech, STRINGS.dupe, STRINGS.guiding, STRINGS.a60, STRINGS.a90, STRINGS.a120}
				for _, k in ipairs(monsterKeywords) do if n:find(k) then table.insert(XRayVision.CachedNPCList, v); break; end end
			else
				local prompt = v:FindFirstChildOfClass("ProximityPrompt")
				local promptMatch = prompt and prompt.ObjectText and (prompt.ObjectText:lower():find(STRINGS.gift) or prompt.ObjectText:lower():find(STRINGS.present) or prompt.ObjectText:lower():find(STRINGS.regalo))
				if promptMatch or itemWhitelistSet[n] or n:find(STRINGS.fuse) or n:find(STRINGS.breaker) or n:find(STRINGS.gift) or n:find(STRINGS.present) or n:find(STRINGS.regalo) then
...
local function updateMainUI()
	...
	local abilityKey = ability and ability[2]
	local isSpeedType = (currentMode == MODES["ABILITIES"] and abilityKey and (abilityKey == "walkspeed" or abilityKey == "gravjump" or abilityKey == "miragespeed"))
	...
	if currentMode == MODES["ABILITIES"] and ability then
		...
		if abilityKey == "walkspeed" then isActive = isWalkSpeedActive
		elseif abilityKey == "miragespeed" then isActive = isMirageSpeedActive
		elseif abilityKey == "gravjump" then isActive = isGravJumpActive
		elseif abilityKey == "supersalto" then isActive = (abilities[currentAbilityIndex][2] == "supersalto" and isSuperSaltoCharged)
		elseif abilityKey == "superhearing" then isActive = SuperHearing.Active
		elseif abilityKey == "xrayvision" then isActive = XRayVision.Active
		elseif abilityKey == "flight" then isActive = isFlying
		elseif abilityKey == "superstrength" then isActive = SuperStrength.Active
		elseif abilityKey == "instantreaction" then isActive = InstantReaction.Active
		end
		local suffix = "    "
		newText = ability[1] .. suffix .. (isActive and " ✓" or "   ")
		UI.abilityText.TextColor3 = (abilityKey == "superstrength" and isActive) and Color3.fromRGB(255, 215, 0) or Color3.fromRGB(255, 255, 255)
...
local function nextSelection()
	if currentMode == MODES["ABILITIES"] then
		...
		until isAbilityAvailable(abilities[currentAbilityIndex][2])
	elseif currentMode == MODES["EMOTES"] then
		if isFlying then
...
local function activateCurrent()
	if currentMode == MODES["ABILITIES"] then
		local ability = abilities[currentAbilityIndex]
		if ability and ability[2] and toggleAbility then
			if ability[2] == "superstrength" and SuperStrength.GrabbedData then
				releaseGrabbedObject(true)
			else
				toggleAbility(ability[2], 1)
			end
		end
	elseif currentMode == MODES["EMOTES"] then
		if isFlying then
...
local function createUI()
	...
	UI.logoImage.MouseButton1Down:Connect(function()
		wasLongPress = false
		local isTouchSpeedHold = UserInputService.TouchEnabled and menuOpen and currentMode == MODES["ABILITIES"] and abilities[currentAbilityIndex][2] == "walkspeed"
		...
		logoHoldTimer = task.delay(holdDuration, function()
			...
			if isTouchSpeedHold then
				...
				elseif abilities[currentAbilityIndex][2] == "supersalto" then
				else
					closeMenu()
				end
			end
		end)
		if menuOpen and abilities[currentAbilityIndex][2] == "supersalto" then startSuperSaltoCharge() end
	end)
	UI.logoImage.MouseButton1Up:Connect(function()
		if logoHoldTimer then task.cancel(logoHoldTimer); logoHoldTimer = nil end
		if abilities[currentAbilityIndex][2] == "supersalto" then releaseSuperSalto() end
	end)
	UI.logoImage.Activated:Connect(function()
		...
		if currentMode == MODES["ABILITIES"] and abilities[currentAbilityIndex][2] == "superstrength" and SuperStrength.GrabbedData then
			releaseGrabbedObject(true)
		else
			if abilities[currentAbilityIndex][2] == "supersalto" then return end
			activateCurrent()
		end
	end)
	...
	UI.abilityText.MouseButton1Down:Connect(function()
		if currentMode ~= MODES["ABILITIES"] or abilities[currentAbilityIndex][2] ~= "walkspeed" then return end
...
local function setupGestures()
	...
	local keyboardMenu = UserInputService.InputBegan:Connect(function(input, gp)
		...
		elseif input.KeyCode == Enum.KeyCode.End then
			if menuOpen and currentMode == MODES["ABILITIES"] and abilities[currentAbilityIndex][2] == "supersalto" then
	...
	local keyboardEndRelease = UserInputService.InputEnded:Connect(function(input, gp)
		...
		if input.KeyCode == Enum.KeyCode.End then
			if menuOpen and currentMode == MODES["ABILITIES"] and abilities[currentAbilityIndex][2] == "supersalto" then releaseSuperSalto() end
		end
	end)
...
local function waitForAnimate(character, timeout)
	...
	while tick() - startTime < timeout do
		local animate = character:FindFirstChild(STRINGS.Animate)
		if animate and animate:FindFirstChild(STRINGS.run) and animate:FindFirstChild(STRINGS.idle) and animate:FindFirstChild(STRINGS.jump) and animate:FindFirstChild(STRINGS.fall) then
			return animate
		end
		task.wait()
	end
...
local function setAnimation(animationType, animationId, animate)
	...
	pcall(function()
		...
		if animationType == "Idle" then
			local idle = findChild(animate, STRINGS.idle)
			if idle then
				local anim1 = idle:FindFirstChild(STRINGS.Animation1)
				local anim2 = idle:FindFirstChild(STRINGS.Animation2)
				if anim1 then anim1.AnimationId = idStr end
				if anim2 then anim2.AnimationId = idStr end
			end
		elseif animationType == "Walk" then
			local walk = findChild(animate, STRINGS.walk)
			local walkAnim = walk and (walk:FindFirstChild(STRINGS.WalkAnim) or walk:FindFirstChild(STRINGS.walk))
			if walkAnim then walkAnim.AnimationId = idStr end
		elseif animationType == "Run" then
			local run = findChild(animate, STRINGS.run)
			local runAnim = run and (run:FindFirstChild(STRINGS.RunAnim) or run:FindFirstChild(STRINGS.run))
			if runAnim then runAnim.AnimationId = idStr end
		elseif animationType == "Jump" then
			local jump = findChild(animate, STRINGS.jump)
			local jumpAnim = jump and (jump:FindFirstChild(STRINGS.JumpAnim) or jump:FindFirstChild(STRINGS.jump))
		...
		elseif animationType == "Fall" then
			local fall = findChild(animate, STRINGS.fall)
			local fallAnim = fall and (fall:FindFirstChild(STRINGS.FallAnim) or fall:FindFirstChild(STRINGS.fall))
		...
		elseif animationType == "Climb" then
			local climb = findChild(animate, STRINGS.climb)
			local climbAnim = climb and (climb:FindFirstChild(STRINGS.ClimbAnim) or climb:FindFirstChild(STRINGS.climb))
			if climbAnim then climbAnim.AnimationId = idStr end
		elseif animationType == "Swim" then
			local swim = findChild(animate, STRINGS.swim)
			local swimAnim = swim and (swim:FindFirstChild(STRINGS.swim) or swim:FindFirstChild(STRINGS.SwimAnim))
			if swimAnim then swimAnim.AnimationId = idStr end
		elseif animationType == "SwimIdle" then
			local swimIdle = findChild(animate, STRINGS.swimidle)
			local swimIdleAnim = swimIdle and (swimIdle:FindFirstChild(STRINGS.swimidle) or swimIdle:FindFirstChild(STRINGS.SwimIdleAnim))
...
local function saveOriginalAnimations(animate)
	...
	if animate[STRINGS.idle] then
		local anim1 = animate[STRINGS.idle]:FindFirstChild(STRINGS.Animation1)
		local anim2 = animate[STRINGS.idle]:FindFirstChild(STRINGS.Animation2)
		if anim1 then originalAnimations.Idle1 = anim1.AnimationId end
		if anim2 then originalAnimations.Idle2 = anim2.AnimationId end
	end
	if animate[STRINGS.walk] then
		local walkAnim = animate[STRINGS.walk]:FindFirstChild(STRINGS.WalkAnim)
		if walkAnim then originalAnimations.Walk = walkAnim.AnimationId end
	end
	if animate[STRINGS.run] then
		local runAnim = animate[STRINGS.run]:FindFirstChild(STRINGS.RunAnim)
		if runAnim then originalAnimations.Run = runAnim.AnimationId end
	end
	if animate[STRINGS.jump] then
		local jumpAnim = animate[STRINGS.jump]:FindFirstChild(STRINGS.JumpAnim)
		if jumpAnim then originalAnimations.Jump = jumpAnim.AnimationId end
	end
	if animate[STRINGS.fall] then
		local fallAnim = animate[STRINGS.fall]:FindFirstChild(STRINGS.FallAnim)
		if fallAnim then originalAnimations.Fall = fallAnim.AnimationId end
	end
	if animate[STRINGS.climb] then
		local climbAnim = animate[STRINGS.climb]:FindFirstChild(STRINGS.ClimbAnim)
		if climbAnim then originalAnimations.Climb = climbAnim.AnimationId end
	end
	if animate:FindFirstChild(STRINGS.swim) then
		local swimAnim = animate[STRINGS.swim]:FindFirstChild(STRINGS.swim)
		if swimAnim then originalAnimations.Swim = swimAnim.AnimationId end
	end
	if animate:FindFirstChild(STRINGS.swimidle) then
		local swimIdleAnim = animate[STRINGS.swimidle]:FindFirstChild(STRINGS.swimidle)
...
loadAnimations = function()
	...
	local animTypes = {"Idle", "Walk", "Run", "Jump", "Fall", STRINGS.Climb, STRINGS.Swim, STRINGS.SwimIdle}
...
startMainAnimationLoop = function()
	local conn = RunService.Heartbeat:Connect(function(deltaTime)
		...
		if humanoid and hrp then
			isCrouching = character:GetAttribute(STRINGS.Crouching) == true or (humanoid.WalkSpeed < 9)
			...
			if isSeated or isClimbing or isSwimming or isExternal or isCrouching then
				...
				if isSeated then
					local sitIdStr = "rbxassetid://" .. ANIMATIONS[STRINGS.sit]
				...
				if isSwimming then
					...
					local targetSwimAnimId = isSwimmingMoving and ANIMATIONS[STRINGS.swim] or ANIMATIONS[STRINGS.swimidle]
				...
				if isClimbing then
					local climbIdStr = "rbxassetid://" .. tostring(ANIMATIONS[STRINGS.climb])
				...
				if not isHoldingTool then
					if not isCrouching then
						for _, track in ipairs(humanoid:GetPlayingAnimationTracks()) do
							...
							local name = track.Name:lower()
							local isStateAnim = (isClimbing and (name:find(STRINGS.climb) or numId == tostring(ANIMATIONS[STRINGS.climb]))) or
								(isSwimming and (name:find(STRINGS.swim) or numId == tostring(ANIMATIONS[STRINGS.swim]) or numId == tostring(ANIMATIONS[STRINGS.swimidle]))) or
								(isSeated and (track == sitAnimTrack or name:find(STRINGS.sit) or name:find(STRINGS.seat)))
		...
		if humanoid and hrp then
			...
			local currentActiveId = currentAnimTrack and currentAnimTrack.Animation and currentAnimTrack.Animation.AnimationId:match("%d+")
			...
			elseif inAir and not isExternal and not SuperStrength.GrabbedData then
				...
				if not jumpAnimTrack then
					local anim = Instance.new("Animation")					
					anim.AnimationId = "rbxassetid://" .. ANIMATIONS[STRINGS.jump]
					jumpAnimTrack = humanoid:LoadAnimation(anim)
					jumpAnimTrack.Looped = false 
					jumpAnimTrack.Priority = Enum.AnimationPriority.Action4
				end
				if not fallAnimTrack then					
					local anim = Instance.new("Animation")
					anim.AnimationId = "rbxassetid://" .. ANIMATIONS[STRINGS.fall]
					fallAnimTrack = humanoid:LoadAnimation(anim)
					fallAnimTrack.Looped = true 
					fallAnimTrack.Priority = Enum.AnimationPriority.Action4
				end
			...
			elseif isMoving and canPlayWalk then
				...
				local animIdToUse = ANIMATIONS[STRINGS.walk]
				if hurtState > 0 then
				...
				else animIdToUse = ANIMATIONS[STRINGS.walk] end
				...
				if currentAnimId == tostring(hurtWalkAnimId) then					
					...
					if currentAnimId == tostring(ANIMATIONS[STRINGS.walk]) then
			...
			for _, track in ipairs(humanoid:GetPlayingAnimationTracks()) do
				if track.Animation then
					...
					if animId == "rbxassetid://" .. ANIMATIONS[STRINGS.walk] then track:AdjustSpeed(1) end
				end
			end
		end
	end)
...
if not isAbilityAvailable(abilities[currentAbilityIndex][2]) then currentAbilityIndex = 1 end 
