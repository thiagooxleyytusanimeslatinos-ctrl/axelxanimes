...
-- ANIMACIONES
local ANIMATIONS = {
	Idle = 136517279340153, Walk = 16738340646, Run = 70636286183373,
...
local ambientSoundMuterConn = nil
-- OTRAS HABILIDADES
...
local SuperStrength = {
	...
	ThrowForce = 800, AnimTrack = nil, LastActionTime = 0, ActivationTime = 0
}
local grabIndicator = nil
local lastIndicatorTarget = nil
local SuperHearing = {
...
local isSpeedNoclipping = false
local speedNoclipOriginalCollisions = {}
...
local batteryNightCooldownUntil = 0
...
local function canGrabObject(part)
	...
	-- Corregido: Evita agarrar el suelo, pero no los objetos que están sobre él.
	if groundCheck and part == groundCheck.Instance and part.Parent == Workspace then
		return false
	end
	...
	if model and (model:FindFirstChildWhichIsA("VehicleSeat", true) or model:FindFirstChildWhichIsA("Seat", true) or
		model.Name:lower():find("car") or model.Name:lower():find("vehic") or model.Name:lower():find("chassis") or model:FindFirstChild("Drive", true)) then
		isVehicle = true
	end
	if isVehicle then
...
local settingOptions = {"ESTATICO: ON", "SILENCIAR: OFF", "NOSIT: OFF", "BATERIA+VUELO: OFF", "BATERIA INF: OFF", "R6 FLY: 1", "DESTRUIR"}
local settingOptions = {"ESTATICO: ON", "SILENCIAR: OFF", "NOSIT: OFF", "BATERIA+VUELO: OFF", "BATERIA INF: OFF", "R6 FLY: 1", "DESACTIVAR SCRIPT"}
local cheatsEnabled = false
local infiniteBatteryEnabled = false
local UI = {}
...
local function startFlight()
	...
	local flightUpdate = RunService.RenderStepped:Connect(function(deltaTime)
		...
		if isR6 then
			...
			elseif not isExternal and not flightEmoteActive then
				if fwd > 0.1 then
					local currentSpeed = isWalkSpeedActive and walkSpeed or 16
					targetAnim = (currentSpeed > 16 and flightFastAnim or flightForwardAnim)
				elseif fwd < -0.1 then
	...
	local function onFlightInputBegan(input, gameProc)
		...
		if input.UserInputType == Enum.UserInputType.Keyboard then
			...
			elseif key == Enum.KeyCode.LeftAlt then flightMoveState.down = 1
			elseif key == Enum.KeyCode.RightAlt then flightMoveState.up = 1
			end
		end
	end
	local function onFlightInputEnded(input, gameProc)
		...
		if input.UserInputType == Enum.UserInputType.Keyboard then
			...
			elseif key == Enum.KeyCode.LeftAlt then flightMoveState.down = 0
			elseif key == Enum.KeyCode.RightAlt then flightMoveState.up = 0
			end
		end
	end
...
local function restoreAmbientSounds()
	...
	emoteMusicActive = false
	if ambientSoundMuterConn then
		ambientSoundMuterConn:Disconnect()
		ambientSoundMuterConn = nil
	end
...
local function muteAmbientSounds()
	if emoteMusicActive then return end
	emoteMusicActive = true

	local function processSound(sound)
		-- Basic checks: Is it a sound, is it playing, is it NOT our own emote sound?
		...
		-- A sound is considered ambient music if it's long, OR it's a reasonably long loop, OR its name suggests it's music.
		local isLikelyMusic = (sound.TimeLength > 30) or (sound.Looped and sound.TimeLength > 8) or (soundName:match("music") or soundName:match("ambien") or soundName:match("radio") or soundName:match("soundtrack"))

		if isLikelyMusic and not originalAmbientVolumes[sound] then
			-- It's likely music and we haven't stored its volume yet.
	...
	local function scanAndMute()
		for _, descendant in ipairs(Workspace:GetDescendants()) do if descendant:IsA("Sound") then processSound(descendant) end end
		for _, descendant in ipairs(SoundService:GetDescendants()) do if descendant:IsA("Sound") then processSound(descendant) end end
	end

	scanAndMute() -- Scan inicial

	if ambientSoundMuterConn then ambientSoundMuterConn:Disconnect() end
	local lastAmbientCheck = 0
	ambientSoundMuterConn = RunService.Heartbeat:Connect(function()
		local now = tick()
		if now - lastAmbientCheck < 0.25 then return end -- Revisar 4 veces por segundo
		lastAmbientCheck = now
		scanAndMute()
	end)
	table.insert(allConnections, ambientSoundMuterConn)
end
...
playAnimation = function(animId, startTime, speed, loop)
	...
	if hrp and tostring(animId) ~= tostring(hurtAnimId) then
		hrp.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
...
local function toggleMirageSpeed()
	if not scriptActive then return end
	if isMirageSpeedActive then stopMirageSpeedEmote() isMirageSpeedActive = false
	else playMirageSpeedEmote() isMirageSpeedActive = true end
end
...
local function ensureXRayHighlight(char)
	if not char or not char.Parent then return end
	if XRayVision.Highlights[char] and XRayVision.Highlights[char].Parent then
		XRayVision.Highlights[char].Enabled = true
		return
	end
	local highlight = Instance.new("Highlight")
	...
	highlight.DepthMode = Enum.HighlightDepthMode.AlwaysOnTop
	highlight.Parent = char
	XRayVision.Highlights[char] = highlight
end
...
local function updateXRayVision()
	...
	if now - XRayVision.LastScanTime > 2 then
	...
	for _, p in ipairs(Players:GetPlayers()) do
		if p ~= player and p.Character and p.Character:FindFirstChild("HumanoidRootPart") then ensureXRayHighlight(p.Character) end
	end

	if now - XRayVision.LastTransparencyUpdateTime > 0.2 then
...
local function cleanupXRayVision()
	...
	for char, highlight in pairs(XRayVision.Highlights) do if highlight then highlight:Destroy() end end
...
local function startAntiFling()
	...
	antiFlingConnection = RunService.Stepped:Connect(function()
		if not isAntiFlingActive or not character or not character.Parent then stopAntiFling() return end
		if hrp and hrp.Parent and not isFlying and not isWalkSpeedActive then
			hrp.AssemblyLinearVelocity = Vector3.new(0,0,0)
			hrp.AssemblyAngularVelocity = Vector3.new(0,0,0)
		end
		for _, part in ipairs(character:GetDescendants()) do
...
local function updateBattery(deltaTime)
	...
	if infiniteBatteryEnabled then
		if battery < maxBattery then battery = maxBattery end
		updateBatteryUI()
		return
	end

	-- Lógica para el cooldown nocturno al volar con batería 0
	local isNight = Lighting:GetSunDirection().Y <= 0.05
	if batteryNightCooldownUntil > 0 and (not isFlying or not isNight) then
		batteryNightCooldownUntil = 0 -- Cancelar cooldown si ya no se vuela o es de día
	end

	if battery <= 0 and isNight and isFlying and batteryNightCooldownUntil == 0 then
		batteryNightCooldownUntil = tick() + 15 -- Iniciar cooldown de 15 segundos
	end

	if batteryNightCooldownUntil > 0 and tick() < batteryNightCooldownUntil then
		-- Durante el cooldown, no hay ganancia de batería. Solo se aplica el costo.
		local costRate = 0
		if isMirageSpeedActive then costRate = costRate + 1 end
		if XRayVision.Active then costRate = costRate + 1 end
		if isGravJumpActive then costRate = costRate + 2 end
		if isWalkSpeedActive and targetWalkSpeed >= 31 and humanoid and humanoid.MoveDirection.Magnitude > 0.1 then costRate = costRate + 1 end
		battery = battery - (costRate * deltaTime)
		if battery < 0 then battery = 0 end
		updateBatteryUI()
		return -- Salir para evitar la recarga
	elseif batteryNightCooldownUntil > 0 and tick() >= batteryNightCooldownUntil then
		batteryNightCooldownUntil = 0 -- Fin del cooldown
	end
	...
	-- Carga al volar y recarga de emergencia
	if isFlying then
		if cheatsEnabled then
			gainRate = gainRate + 5 -- Carga rápida de batería al volar.
		end
	end

	if battery < 5 then
		-- Recarga de emergencia para no quedar en 0.
		gainRate = gainRate + 1
	end
	...
	if not cheatsEnabled then
		-- Sin trucos, la batería dura un poco más (es más resistente).
		costRate = costRate * 0.7
	end
	...
	if not isBatteryDepleted then
		if battery < 5 then
			-- Disable all abilities if they are active
			...
			if SuperStrength.Active then toggleAbility("superstrength", 1) end
		...
		elseif battery < 10 then
			-- Disable all non-flight abilities if they are active
			if isMirageSpeedActive then toggleAbility("miragespeed", 1) end
			if isGravJumpActive then toggleAbility("gravjump", 1) end
			if SuperHearing.Active then toggleAbility("superhearing", 1) end
			if XRayVision.Active then toggleAbility("xrayvision", 1) end
			if SuperStrength.Active then toggleAbility("superstrength", 1) end
			if InstantReaction.Active then toggleAbility("instantreaction", 1) end
			if isWalkSpeedActive and not isFlying then toggleAbility("walkspeed", 1) end
		end
	end
	...
	if battery == 0 and not isBatteryDepleted then
		...
		-- Activar velocidad fija de "correr"
	...
	elseif battery > 5 and isBatteryDepleted then
		...
		-- Desactivar velocidad fija
		isWalkSpeedActive = false
		targetWalkSpeed = 16 -- Reset to default
...
local function updateMainUI()
	...
	if currentMode == MODES.ABILITIES and ability then
		...
		local suffix = "    "
		if isBatteryDepleted and ability.key == "walkspeed" then
			newText = "Correr" .. suffix .. " ✓"
		else
...
local function activateCurrent()
	...
	elseif currentMode == MODES.SETTINGS then
		...
		elseif option:find("BATERIA+VUELO") then
			cheatsEnabled = not cheatsEnabled
			settingOptions[currentSettingIndex] = "BATERIA+VUELO: " .. (cheatsEnabled and "ON" or "OFF")
		elseif option:find("BATERIA INF") then
			infiniteBatteryEnabled = not infiniteBatteryEnabled
			settingOptions[currentSettingIndex] = "BATERIA INF: " .. (infiniteBatteryEnabled and "ON" or "OFF")
		...
		elseif option == "DESACTIVAR SCRIPT" then
			scriptActive = false
...
local function updateFlightControlsVisibility()
	if not UI.upArrow or not UI.downArrow then return end
	local visible = menuOpen and isFlying
	UI.upArrow.Visible = visible
	UI.downArrow.Visible = visible
end
...
local function openMenu()
	...
	if UI.mainPanel then
		...
		local tween = TweenService:Create(UI.mainPanel, TweenInfo.new(0.2), {BackgroundTransparency = 0.3, Position = UDim2.new(1, -210, 0, 10)})
		tween:Play()
...
local function createUI()
	...
	UI.abilityText.MouseButton1Click:Connect(function()
		if abilityLongPressActive then abilityLongPressActive = false; return end
		nextSelection()
	end)
	UI.abilityText.MouseButton2Click:Connect(function()
		if abilityLongPressActive then abilityLongPressActive = false; return end
		previousSelection()
	end)
	UI.abilityText.TouchTap:Connect(function(touchPositions)
		if abilityLongPressActive then abilityLongPressActive = false; return end
		local touch = touchPositions[1]
		local relativeX = touch.X - UI.abilityText.AbsolutePosition.X
		if relativeX > UI.abilityText.AbsoluteSize.X / 2 then nextSelection() else previousSelection() end
	end)
	...
	UI.upArrow = Instance.new("TextButton")
	UI.upArrow.Name = "UpArrow"
	UI.upArrow.Size = UDim2.new(0, 30, 0, 30)
	UI.upArrow.Position = UDim2.new(0, 185, 0, 57)
	UI.upArrow.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
	UI.upArrow.BackgroundTransparency = 0.5
	UI.upArrow.Text = "▲"
	UI.upArrow.TextColor3 = Color3.new(1, 1, 1)
	UI.upArrow.Font = Enum.Font.GothamBold
	UI.upArrow.TextSize = 20
	UI.upArrow.Visible = false
	UI.upArrow.Parent = UI.mainPanel
	Instance.new("UICorner", UI.upArrow).CornerRadius = UDim.new(0, 8)
	UI.upArrow.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then flightMoveState.up = 1 end end)
	UI.upArrow.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then flightMoveState.up = 0 end end)

	UI.downArrow = Instance.new("TextButton")
	UI.downArrow.Name = "DownArrow"
	UI.downArrow.Size = UDim2.new(0, 30, 0, 30)
	UI.downArrow.Position = UDim2.new(0, 150, 0, 57)
	UI.downArrow.BackgroundColor3 = Color3.fromRGB(50, 50, 50)
	UI.downArrow.BackgroundTransparency = 0.5
	UI.downArrow.Text = "▼"
	UI.downArrow.TextColor3 = Color3.new(1, 1, 1)
	UI.downArrow.Font = Enum.Font.GothamBold
	UI.downArrow.TextSize = 20
	UI.downArrow.Visible = false
	UI.downArrow.Parent = UI.mainPanel
	Instance.new("UICorner", UI.downArrow).CornerRadius = UDim.new(0, 8)
	UI.downArrow.InputBegan:Connect(function(input) if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then flightMoveState.down = 1 end end)
	UI.downArrow.InputEnded:Connect(function(input) if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then flightMoveState.down = 0 end end)
	...
	UI.speedIndicatorLabel.TextSize = 16
...
local function setupGestures()
	...
	local mouseMenuControl = UserInputService.InputBegan:Connect(function(input, gp) -- Solo para M3
		if gp then return end
	...
	local strengthInput = UserInputService.InputBegan:Connect(function(input, gp)
		...
		if input.UserInputType == Enum.UserInputType.Touch then return end -- Se maneja con TouchTap
		if input.UserInputType == Enum.UserInputType.MouseButton1 then
			...
			if SuperStrength.GrabbedData then
				...
				if result and result.Instance then tryGrabObject(result.Instance) end
			end
		end
	end)
...
-- ============ TOGGLE ABILITY ============
local function tryGrabObject(targetPart)
	if not SuperStrength.Active or SuperStrength.GrabbedData or not targetPart then return end
	if tick() - SuperStrength.LastActionTime < 1.5 then return end

	local can, mainPart, model, isV = canGrabObject(targetPart)
	if can then
...
local function updateGrabIndicator()
	if not SuperStrength.Active or SuperStrength.GrabbedData then
		if grabIndicator then
			pcall(function() grabIndicator:Destroy() end)
			grabIndicator = nil
			lastIndicatorTarget = nil
		end
		return
	end
	...
	local targetPart = result and result.Instance

	local can, mainPart, model = targetPart and canGrabObject(targetPart)

	local currentTarget = model or mainPart

	if can and currentTarget then
		if lastIndicatorTarget ~= currentTarget then
			if grabIndicator then pcall(function() grabIndicator:Destroy() end) end
			
			grabIndicator = Instance.new("Highlight")
			grabIndicator.Name = "SuperGrabIndicator"
			grabIndicator.FillColor = Color3.fromRGB(0, 120, 255)
			grabIndicator.OutlineColor = Color3.fromRGB(0, 255, 255)
			grabIndicator.FillTransparency = 0.7
			grabIndicator.OutlineTransparency = 0.2
			grabIndicator.DepthMode = Enum.HighlightDepthMode.Occluded
			grabIndicator.Parent = currentTarget
			lastIndicatorTarget = currentTarget
		end
	elseif grabIndicator then
		pcall(function() grabIndicator:Destroy() end)
		grabIndicator = nil
		lastIndicatorTarget = nil
	end
...
local function toggleSuperStrength()
	...
	local function disconnectVehicleOccupants(model)
		...
		for _, seat in ipairs(model:GetDescendants()) do
			if seat:IsA("VehicleSeat") or seat:IsA("Seat") then
				savedStates[seat] = {Disabled = seat.Disabled, Occupant = seat.Occupant, CanCollide = seat.CanCollide}
				seat.Disabled = true
	...
	if not SuperStrength.Active then
		...
		if SuperStrength.Connection then SuperStrength.Connection:Disconnect() SuperStrength.Connection = nil end
		if grabIndicator then pcall(function() grabIndicator:Destroy() end) grabIndicator = nil; lastIndicatorTarget = nil end
	else
...
toggleAbility = function(key, mode)
	...
	if isBatteryDepleted then return end -- Handles battery at 0

	-- Determine if we are activating an ability
	local isCurrentlyActive = false
	if key == "walkspeed" then isCurrentlyActive = isWalkSpeedActive
	elseif key == "miragespeed" then isCurrentlyActive = isMirageSpeedActive
	elseif key == "gravjump" then isCurrentlyActive = isGravJumpActive
	elseif key == "superhearing" then isCurrentlyActive = SuperHearing.Active
	elseif key == "xrayvision" then isCurrentlyActive = XRayVision.Active
	elseif key == "superstrength" then isCurrentlyActive = SuperStrength.Active
	elseif key == "instantreaction" then isCurrentlyActive = InstantReaction.Active
	elseif key == "flight" then isCurrentlyActive = isFlying
	end

	-- If we are trying to activate an ability, check battery level
	if not isCurrentlyActive and battery < 10 and not (key == 'flight' and battery >= 5) then
		return -- Blocked: activating any other ability below 10, or flight below 5
	end
...
adjustSpeed = function(increase)
	...
	if not scriptActive or not character or not character.Parent or not humanoid or humanoid.Health <= 0 then return end
	if isBatteryDepleted then return end
	if battery == 0 and isWalkSpeedActive then
		return
	end
	if isWalkSpeedActive then
		if increase then
	...
	elseif isGravJumpActive then
		local gravityStep = 20
		local minGravity = -500
		local maxGravity = 500
		if increase then
...
startMainAnimationLoop = function()
	local conn = RunService.Heartbeat:Connect(function(deltaTime)
		...
		if humanoid and hrp then
			...
			if not isR6 and animate and not animate.Disabled and not isHoldingTool and not isCrouching then
				animate.Disabled = true
				-- Al desequipar un item, forzamos la detención de animaciones para evitar que la pose se quede congelada.
				for _, track in ipairs(humanoid:GetPlayingAnimationTracks()) do
					local numId = track.Animation.AnimationId:match("%d+")
					if not managedAnimationIds[numId] and track.Priority.Value < Enum.AnimationPriority.Action3.Value then track:Stop(0.1) end
				end
			elseif isHoldingTool and animate and animate.Disabled then
				-- Reactivamos el script de animación por defecto al sostener un objeto.
				-- Esto es necesario para que funcionen las animaciones y la UI de los items del juego.
				animate.Disabled = false
			end
			...
			elseif isMoving and canPlayWalk then
				...
				if effectiveSpeed >= 50 then animIdToUse = 78510387198062
				elseif effectiveSpeed >= 23 then animIdToUse = ANIMATIONS.Run
				elseif effectiveSpeed > 16 then animIdToUse = 72301599441680
				...
				if isWalkSpeedActive then
					if effectiveSpeed >= 31 and effectiveSpeed <= 41 then
						animSpeedMultiplier = 1.5
					elseif effectiveSpeed == 20 then animSpeedMultiplier = 1
					elseif effectiveSpeed == 23 then animSpeedMultiplier = 1
					elseif effectiveSpeed >= 45 and effectiveSpeed < 50 then animSpeedMultiplier = 2.8
					elseif effectiveSpeed >= 50 then animSpeedMultiplier = 3.5 + (effectiveSpeed / 50)
					end
				end
				...
				if hurtState > 0 and not isEmotePlaying and not isCrouching then
					...
					if not isEmotePlaying and not isCrouching and not isMirageSpeedActive and not isExternal then
						...
						elseif not isHoldingTool and not (currentAnimTrack and currentAnimTrack.IsPlaying and currentAnimTrack.Animation and currentAnimTrack.Animation.AnimationId == "rbxassetid://" .. idleAnimId) then
...
if isR6 and currentMode == MODES.EMOTES then currentMode = MODES.ABILITIES end
if not isR6 and settingOptions[currentSettingIndex]:find("R6 FLY") then currentSettingIndex = 1 end
updateMainUI()
